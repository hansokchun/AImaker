...
create policy "Clients can update unpaid proposal decisions"
  on public.proposals for update
  using (auth.uid() = client_id and payment_status = 'unpaid')
  with check (auth.uid() = client_id and payment_status = 'unpaid' and status in ('revision_requested', 'cancelled'));

drop policy if exists "Experts can update own unpaid proposals" on public.proposals;
create policy "Experts can update own unpaid proposals"
  on public.proposals for update
  using (auth.uid() = expert_id and payment_status = 'unpaid')
  with check (auth.uid() = expert_id and payment_status = 'unpaid' and status in ('sent', 'cancelled'));
...
begin
  ...
  raise exception 'work update is not allowed for this state transition';
end;
$$;
