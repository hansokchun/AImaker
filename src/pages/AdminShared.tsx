...
const statusClass = (status?: string): string => {
    if (status === 'published' || status === 'paid' || status === 'completed' || status === 'accepted' || status === 'active' || status === 'featured' || status === 'resolved' || status === 'settled') return 'is-live';
    if (status === 'open' || status === 'submitted' || status === 'sent' || status === 'in_progress' || status === 'pending' || status === 'held') return 'is-open';
...
const statusLabel = (status?: string): string => {
    const labels: Record<string, string> = {
        ...
        hide_review: '리뷰 숨김',
        in_progress: '진행 중',
        mark_refund_pending: '환불 대기',
        mark_settlement_pending: '정산 대기',
        mark_settlement_settled: '정산 완료',
