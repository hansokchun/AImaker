...
describe('Home', () => {
  ...
  it('keeps the Stitch home structure with image-backed category buttons and focused copy', async () => {
    ...
    expect(screen.getByRole('heading', { name: /AI로 더 싸고 빠르게,\s*필요한 작업을 골라보세요/ })).toBeInTheDocument()
    ...
    expect(screen.getByRole('link', { name: 'AI 숏폼 영상' })).toHaveAttribute('href', '/category?q=AI%20%EC%88%8F%ED%8F%BC%20%EC%98%81%EC%83%81')
    expect(screen.getByRole('link', { name: '상세페이지 이미지' })).toHaveAttribute('href', '/category?q=%EC%83%81%EC%84%B8%ED%8E%98%EC%9D%B4%EC%A7%80%20%EC%9D%B4%EB%AF%B8%EC%A7%80')
    ...
    expect(screen.getByText('썸네일, 상세페이지, 캐릭터 이미지')).toBeInTheDocument()
    expect(screen.getByText('자동화, 챗봇, 업무용 웹 도구')).toBeInTheDocument()
    ...
    expect(screen.queryByText('요구사항 전달부터 결제, 프로젝트까지 한 곳에서 관리할 수 있습니다.')).not.toBeInTheDocument()
