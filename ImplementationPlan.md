...
- [ ] `vitest`, `@testing-library/react`, `@testing-library/jest-dom`, `jsdom`을 dev dependency로 추가한다.
- [ ] `package.json`에 `test` 스크립트를 추가한다.
- [ ] `vite.config.ts`에 Vitest 설정을 추가한다.
- [ ] `src/test/setup.ts`에서 `@testing-library/jest-dom`을 불러온다.
...
- [ ] `AiCategory`, `ExpertProduct`, `ProductPackage`, `ServiceRequest`, `Proposal`, `Work`, `WorkStep`, `Deliverable`, `Review` 타입을 추가한다.
- [ ] 초기 3개 카테고리를 상수로 분리한다.
- [ ] 제안서 유효기간 3일, 외부 연락 금지 문구를 정책 상수로 분리한다.
- [ ] mock data를 상품 중심으로 재구성한다.
...
- [ ] 핵심 문구를 `AI 외주를 더 쉽고 저렴하게`로 교체한다.
- [ ] 3개 초기 카테고리 섹션을 추가한다.
- [ ] 추천 AI 상품 섹션을 추가한다.
- [ ] `AI 작업 맡기기`, `AI 전문가로 시작하기` CTA를 배치한다.
...
- [ ] 목록의 기본 단위를 전문가가 아니라 상품으로 바꾼다.
- [ ] 상품 카드에 상품명, 시작가, 작업 기간, 사용 AI 도구, 샘플 정보를 표시한다.
- [ ] 가격 범위, AI 도구 필터를 유지하거나 단순화한다.
- [ ] 상품이 없을 때 빈 상태 문구를 표시한다.
...
- [ ] 상품 설명, 샘플 결과물, 사용 AI 도구를 상단에서 강조한다.
- [ ] Standard 패키지를 필수로 표시한다.
- [ ] `패키지로 의뢰하기` 버튼을 `/request/:productId`로 연결한다.
- [ ] 바로 결제가 아니라 요구사항 작성으로 이동한다는 안내를 표시한다.
...
- [ ] 선택한 패키지 요약을 표시한다.
- [ ] 원하는 결과물, 작업 목적, 참고자료, 마감 희망일 입력을 추가한다.
- [ ] 단일 진행/단계별 진행 선택을 추가한다.
- [ ] 연락처 입력 필드를 제거한다.
- [ ] 외부 연락 금지 안내 문구를 표시한다.
...
- [ ] 제안서 상세 화면을 추가한다.
- [ ] 최종 금액, 작업 범위, 작업 기간, 산출물, 수정 횟수를 표시한다.
- [ ] 제안서 유효기간 3일 안내를 표시한다.
- [ ] 승인, 수정 요청, 취소 버튼을 추가한다.
- [ ] 승인 전에는 작업이 시작되지 않는다는 문구를 표시한다.
...
- [ ] 단일 진행도 하나의 단계로 표시한다.
- [ ] 단계별 상태를 표시한다.
- [ ] 산출물 외부 링크 제출 UI를 추가한다.
- [ ] 의뢰자 승인/수정 요청 UI를 추가한다.
...
- [ ] 상품 등록에 필요한 최소 입력을 정리한다.
- [ ] 사용 AI 도구, 샘플 결과물, 상품 설명, 시작 가격, 작업 기간을 입력받는다.
- [ ] Standard 패키지를 필수로 둔다.
- [ ] Deluxe/Premium은 선택으로 둔다.
...
- [ ] 의뢰자 섹션을 추가한다.
- [ ] 전문가 섹션을 추가한다.
- [ ] 내 의뢰 요청, 받은 제안서, 진행 중인 작업을 연결한다.
- [ ] 내가 등록한 상품, 받은 요청, 보낸 제안서를 연결한다.
- [ ] 완료된 작업에만 리뷰 작성 버튼을 표시한다.
...
- [ ] `SupabasePlan.md` 기준으로 새 테이블 구조를 반영한다.
- [ ] 기존 `profiles` 구조와 충돌하지 않게 정리한다.
- [ ] `expert_products`, `proposals`, `works`, `work_steps`, `deliverables`, `reviews`를 추가한다.
- [ ] RLS 정책을 작성한다.
- [ ] Storage bucket 정책을 문서 기준으로 반영한다.
