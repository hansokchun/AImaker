# 기그온 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 기그온을 AI 상품 탐색, 요구사항 작성, 거래 제안서, 작업 진행표까지 이어지는 초기 런칭 가능한 서비스로 개편한다.
...
## 2026-06-17 최신 수정: 결제 기능 후순위 전환

테스트 런칭 단계에서는 실제 결제, PG 연동, 모의 결제 버튼을 모두 후순위로 미룬다. 결제 기능이 있으면 테스트 계정으로 사용자 흐름을 반복 검증할 때 불필요한 마찰이 생기므로, 첫 검증 목표는 결제가 아니라 상품 탐색부터 작업방 진행까지의 거래 흐름 안정화로 둔다.

### 수정된 우선순위

1. 로그인, 프로필, 샘플, 상품 데이터 검증
2. 상품 탐색, 요구사항 작성, 제안서, 작업방 흐름
3. 관리자페이지 MVP
4. 공통 디자인 시스템과 모바일 사용성
5. Supabase RLS, Storage, 테스트 데이터 안정화
6. 결제, 정산, 환불, 세금계산서 기능

### 개발 기준

- 제안서 수락 시 결제 페이지로 보내지 않고 바로 작업방을 생성하거나 작업방으로 이동한다.
- 결제/정산 관련 데이터 모델은 미래 확장을 위해 남길 수 있지만, 테스트 런칭의 필수 UI나 필수 플로우로 노출하지 않는다.
- 관리자페이지는 우선 회원, 상품, 요청, 제안서, 작업방, 신고/검수 중심으로 만든다.
- 결제 상태, 정산 상태는 당분간 운영 메모 또는 미래 기능 placeholder로만 취급한다.
- 기존 문서에 "첫 런칭 결제 포함"이라고 적힌 내용보다 이 섹션의 결정이 우선한다.

### 테스트 기준 변경

```text
제안서 수락 -> 결제 페이지 -> 결제 완료 -> 작업방 생성
```

위 흐름은 후순위로 미룬다. 테스트 런칭에서는 다음 흐름을 검증한다.

```text
제안서 수락 -> 작업방 생성 -> 흐름설계 -> 결과물 제출 -> 결과물 승인
```
